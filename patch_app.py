...
imports_match = re.search(r"import \{([^}]+)\} from 'lucide-react';", content)
if imports_match:
    imports_str = imports_match.group(1)
    new_imports = set(i.strip() for i in imports_str.split(','))
    for i in ['LayoutDashboard', 'BarChart3', 'Wallet', 'Package', 'Tag', 'Users', 'Clock', 'Calendar', 'Bell', 'FileText', 'Settings']:
        new_imports.add(i)
    new_imports = [i for i in new_imports if i]
    content = content.replace(imports_match.group(0), f"import {{ {', '.join(new_imports)} }} from 'lucide-react';")

# Update state type in App.tsx
content = content.replace("useState<'dashboard' | 'settings'>('dashboard');", "useState<'overview' | 'reports' | 'cajas' | 'inventory' | 'promos' | 'agents' | 'time' | 'calendar' | 'alerts' | 'sri' | 'settings'>('overview');")

# Find the admin layout section
layout_regex = re.compile(
    r"\{\/\* ADMINISTRATOR ADVANCED COHESIVE SIDEBAR WORKSPACE \*\/\}.*?\{\/\* Mobile Bottom Navigation for Admin \*\/\}.*?/>\s*</div>\s*\)\s*\}",
    re.DOTALL
)

new_layout = """{/* ADMINISTRATOR ADVANCED COHESIVE SIDEBAR WORKSPACE */}
